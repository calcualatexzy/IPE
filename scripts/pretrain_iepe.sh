#!/usr/bin/env bash
# Single-GPU RunAI IEPE pretraining; run inside the submitted container.
# Usage: bash scripts/pretrain_iepe.sh [SUFFIX] [DATASET_PATH] [HYDRA_OVERRIDES...]
...
printf 'Starting IEPE on one GPU\nDataset: %s\nOutput: %s\nSuffix: %s\n' \
...
exec torchrun --standalone --nproc_per_node=1 train.py \
  ...
  "hydra.run.dir=$OUTPUT_DIR/hydra/iepe/\${now:%Y-%m-%d_%H-%M-%S}" \
